...
public class HoloDevice : MonoBehaviour
{
  ...
  // Set a user's render mode
  public bool SetUser2D(int user, bool render2D = true)
  {
    if (!IsUserValid(user))
      return false;
    m_renderCave.m_render2D = render2D;
    return true;
  }
  ...
  void Initialise()
  {
    ...
    bool showCalibrationScreen = false;
    ...
    if (holoCanvas)
    {
      ...
      if (calibController != null)
        calibController.gameObject.SetActive(showCalibrationScreen);
    }
  ...
  private void UpdateVRPNDevices()
  {
    ...
    for (int i = 0; i < m_users.Length; ++i)
    {
      ...
      m_users[i].wand.m_forceTCP =  DeviceConfig.VRPNOverTCP;
      m_users[i].glasses.m_forceTCP =  DeviceConfig.VRPNOverTCP;
    }

    m_trackingEvents.m_enabled  = DeviceConfig.EnableTracking;
    m_trackingEvents.m_server   = vrpnServer;
    m_trackingEvents.m_forceTCP = DeviceConfig.VRPNOverTCP;
