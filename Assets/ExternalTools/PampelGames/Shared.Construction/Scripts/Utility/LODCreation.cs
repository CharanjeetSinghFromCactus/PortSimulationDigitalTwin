...
namespace PampelGames.Shared.Construction
{
    ...
    {
        public static void CreateLODs(ConstructionPool pool, SceneObjectBase sceneObject, List<float> lodList, ShadowCastingMode shadowCastingMode)
        {
            var gameObject = sceneObject.gameObject;
            if (gameObject.TryGetComponent<LODGroup>(out _)) return;

            ObjectUtility.CreateLODObj(pool, shadowCastingMode, gameObject, 0,
            ...
            ObjectUtility.DestroyObject(pool, sceneObject.meshFilter);
            ObjectUtility.DestroyObject(pool, sceneObject.meshRenderer);
            ...
            for (var i = 1; i < lodList.Count; i++)
            {
                ...
                ObjectUtility.CreateLODObj(pool, shadowCastingMode, gameObject, i,
