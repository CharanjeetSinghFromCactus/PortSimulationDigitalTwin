...
{
    public static class AngleDistanceUtility
    {
        private const float AngleDistanceTolerance = 90f; // Degrees
        private const float AngleDistanceFactor = 0.0175f;
        ...
        /// <param name="widthExisting">Width of the existing track.</param>
        /// <param name="widthConnecting">Width of the new connecting track.</param>
        ...
        public static float GetAngleDistance(float intersectionDistance, float widthExisting, float widthConnecting, float closestAngle)
        {
            ...
            if (math.degrees(closestAngle) >= AngleDistanceTolerance) return distance;
            var additionalDistance = (AngleDistanceTolerance - math.degrees(closestAngle)) * widthConnecting *
                                     AngleDistanceFactor;
        ...
        public static float3 ApplyAngleSettings(float3 tangentIn, float3 tangentInLeft, float3 tangentInRight, float minAngle, float snapAngle)
        {
            ...
            if (math.abs(angleToLeft) < minAngle) tangentIn = PGTrigonometryUtility.RotateTangentXZ(tangentIn, angleToLeft - minAngle);

            if (math.abs(angleToRight) < minAngle) tangentIn = PGTrigonometryUtility.RotateTangentXZ(tangentIn, -(math.abs(angleToRight) - minAngle));
