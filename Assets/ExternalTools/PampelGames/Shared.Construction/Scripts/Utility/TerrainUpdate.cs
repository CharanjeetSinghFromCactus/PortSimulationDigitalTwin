...
namespace PampelGames.Shared.Construction
{
    ...
    public static class TerrainUpdate
    {
            ...
            bool resetTerrain = false, TerrainCache resetTerrainUndo = null)
        {
            ...
            {
                if (splinePositionsArrayMiddle.Length <= i || splinePositionsArrayLeft.Length <= i) break;
            ...
            if (settings.removeDetails)
            {
                ...
                for (var layer = 0; layer < terrainData.detailPrototypes.Length; layer++)
                {
                    ...
                    if (resetTerrain)
                    {
                        ...
                        var resetDetailLayerLength0 = resetDetailLayer.GetLength(0);
                        var resetDetailLayerLength1 = resetDetailLayer.GetLength(1);
                        ...
                        {
                            var valueRow = minY + y;
                            var valueColumn = minX + x;
                            if (valueRow < 0 || valueColumn < 0 || valueRow >= resetDetailLayerLength0 ||
                                valueColumn >= resetDetailLayerLength1) continue;
                            subResetDetailLayer[y, x] = resetDetailLayer[valueRow, valueColumn];
                        }
