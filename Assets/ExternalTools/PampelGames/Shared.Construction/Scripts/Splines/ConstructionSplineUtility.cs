...
namespace PampelGames.Shared.Construction
{
    public static class ConstructionSplineUtility
    {
        ...
        public static void ScaleSpline(Spline spline, float3 scale, float3 center)
        {
            for (var i = 0; i < spline.Count; i++)
            {
                var knot = spline[i];
                var offset = knot.Position - center;
                offset *= scale;
                knot.Position = center + offset;
        ...
        public static bool IsClockwise(Spline spline)
        {
            var area = 0f;
            for (var i = 0; i < spline.Count; i++)
            {
                var p1 = spline[i].Position;
                var p2 = spline[(i + 1) % spline.Count].Position;
                var a = new float2(p1.x, p1.z);
                var b = new float2(p2.x, p2.z);
        ...
        public static void SetClockwise(Spline spline)
        {
            if (!IsClockwise(spline)) InvertSpline(spline);
        }
