...
namespace PampelGames.Shared.Construction
{
    ...
    {
        public static SplineEdge[] CreateLine(float width, bool backface, float rotationDegrees = 0f)
        {
            ...
            var leftPos = new float2(-halfWidth, 0f);
            var rightPos = new float2(halfWidth, 0f);
            var leftRotated = new float2(leftPos.x * cos - leftPos.y * sin, leftPos.x * sin + leftPos.y * cos);
            var rightRotated = new float2(rightPos.x * cos - rightPos.y * sin, rightPos.x * sin + rightPos.y * cos);
            splineEdges[0] = new SplineEdge(leftRotated, 0f, -rotationDegrees);
            splineEdges[1] = new SplineEdge(rightRotated, 1f, -rotationDegrees, true);
            if (!backface) return splineEdges;
            splineEdges[2] = new SplineEdge(rightRotated, 0f, 180f - rotationDegrees);
            splineEdges[3] = new SplineEdge(leftRotated, 1f, 180f - rotationDegrees);
            return splineEdges;
        }
        ...
        public static SplineEdge[] CreateCircle(float radius, int faceCount, float rotationDegrees = 0f)
        {
            if (faceCount <= 1) return CreateLine(radius * 2f, false, rotationDegrees);
            if (faceCount == 2) return CreateLine(radius * 2f, true, rotationDegrees);
            ...
            {
                var angle = i * angleStep + math.PI / 2f + rotationRadians;
                var x = radius * math.cos(angle);
                var y = radius * math.sin(angle);
                var uvX = (float) i / faceCount;
                var normalRotation = i * (360f / faceCount) - rotationDegrees;
