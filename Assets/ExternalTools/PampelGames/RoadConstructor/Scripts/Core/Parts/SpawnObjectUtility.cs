    ...
    internal static class SpawnObjectUtility
    {
            ...
            List<int> overlapIntersectionIndexes, List<int> overlapRoadIndexes)
        {
            ...
            for (var i = 0; i < roads.Count; i++)
            {
                ...
                for (var j = newOverlapIntersectionIndexes.Count - 1; j >= 0; j--)
                {
            ...
            void ProcessRoad(RoadObject _road, List<int> _overlapIntersectionIndexes)
            {
                ...
                _road.spawnedConstructionObjects.AddRange(spawns);
            }
            ...
            void ProcessIntersection(IntersectionObject _intersection, List<int> _overlapIntersectionIndexes)
            {
                ...
                // Replaced roads may be inside the overlap roads, remove them.
                var _overlapRoadIndexes = new List<int>(overlapRoadIndexes);
                var _roadConnections = _intersection.RoadConnections;
                var minLength = _intersection.roadDescr.settings.roadLength.x;
                var _combinedConnections = new List<RoadObject>();
                for (var i = 0; i < _roadConnections.Count; i++)
                    _combinedConnections.AddRange(ConnectionUtility.GetConnectingRoads(_roadConnections[i], minLength));
                for (var i = _overlapRoadIndexes.Count - 1; i >= 0; i--)
                {
                    var _roadConnection = roadObjects[_overlapRoadIndexes[i]];
                    if (_combinedConnections.Contains(_roadConnection)) _overlapRoadIndexes.RemoveAt(i);
                }

                spawns.AddRange(SpawnIntersectionApproach(spawnObjectPresets, _intersection,
                    roadObjects, intersectionBounds, _overlapIntersectionIndexes, _overlapRoadIndexes));
                ...
                _intersection.spawnedConstructionObjects.AddRange(spawns);
            }
            ...
            void ProcessSceneObject(SceneObject sceneObject)
            {
                ...
                sceneObject.spawnedConstructionObjects.AddRange(spawns);
            }
            ...
            var tempOverlapRoadIndexes = new List<int>();
            for (var i = 0; i < roads.Count; i++)
            {
                tempNewRoads.Add(roads[i]);
                tempOverlapRoadIndexes.Add(i);
            }

            var tempOverlapIntersectionIndexes = new List<int>();
            var tempIntersectionBounds = new List<Bounds>();
            for (var i = 0; i < intersections.Count; i++)
            {
                tempOverlapIntersectionIndexes.Add(i);
            ...
            for (var i = 0; i < overlapRoadIndexes.Count; i++)
            {
                ...
                for (var j = spawnedObjects.Count - 1; j >= 0; j--)
                {
                    ...
                    if (CheckOverlap(overlapObj.gameObject, tempNewRoads, tempIntersectionBounds, tempOverlapIntersectionIndexes,
                            tempOverlapRoadIndexes)) continue;
                    spawnedObjects.RemoveAt(j);
                    ObjectUtility.DestroyObject(overlapRoad.roadDescr.roadConstructor.pool, overlapObj);
                }
            }
            ...
            for (var i = 0; i < overlapIntersectionIndexes.Count; i++)
            {
                ...
                // Replaced roads may be inside the overlap roads, remove them.
                var _newRoads = new List<RoadObject>(tempNewRoads);
                var _roadConnections = overlapIntersection.RoadConnections;
                var minLength = overlapIntersection.roadDescr.settings.roadLength.x;
                var _combinedConnections = new List<RoadObject>();
                for (var j = 0; j < _roadConnections.Count; j++)
                    _combinedConnections.AddRange(ConnectionUtility.GetConnectingRoads(_roadConnections[j], minLength));
                for (var j = _newRoads.Count - 1; j >= 0; j--)
                {
                    var _roadConnection = _newRoads[j];
                    if (_combinedConnections.Contains(_roadConnection)) _newRoads.RemoveAt(j);
                }

                var _overlapRoadIndexes = new List<int>();
                for (var j = 0; j < _newRoads.Count; j++) _overlapRoadIndexes.Add(j);
                ...
                for (var j = spawnedObjects.Count - 1; j >= 0; j--)
                {
                    ...
                    if (!overlapObj.spawnObject.removeOverlap) continue;
                    if (CheckOverlap(overlapObj.gameObject, _newRoads, tempIntersectionBounds, tempOverlapIntersectionIndexes,
                            _overlapRoadIndexes)) continue;

                    spawnedObjects.RemoveAt(j);
                    ObjectUtility.DestroyObject(overlapIntersection.roadDescr.roadConstructor.pool, overlapObj);
                }
            }
        }
            ...
            List<int> overlapIntersectionIndexes, List<int> overlapRoadIndexes)
        {
            ...
            if (road.curvature < spawnObject.curvature.x || road.curvature > spawnObject.curvature.y) return spawnedObjects;

            var settings = road.roadDescr.settings;
            ...
            void Process(Spline middleSpline, Spline _spline, bool otherSide)
            {
                ...
                var evaluations = PGMathUtility.GetEvenlySpacedValues(length, spacing, out var gap);

                for (var i = 0; i < evaluations.Count; i++)
                {
                    var spawnChance = CheckSpawnChance(settings, spawnObject.chance);
                    if (!spawnChance) continue;

                    var t = evaluations[i];
                    t += spawnObject.positionOffsetForward / length;
                    if (t is > 1f or < 0f) continue;
...
#endif
                    if (checkElevation &&
                        !CheckElevation(road.roadDescr.settings, position, spawnObject.heightRange, spawnObject.elevation)) continue;
                    ...
                    if (spawnObject.removeOverlap)
                        ...
                        {
                            ObjectUtility.DestroyObject(road.roadDescr.roadConstructor.pool, spawnedObject);
                            continue;
                        }
            ...
            List<int> overlapIntersectionIndexes, List<int> overlapRoadIndexes)
        {
            ...
            for (var i = 0; i < roads.Count; i++)
            {
                ...
                for (var j = 0; j < _spawnObjects.Count; j++)
                {
                    ...
                    if (spawnObject.intersectionExitType == IntersectionExitType.Intersection && intersection.RoadConnections.Count < 3)
                        continue;
                    if (spawnObject.intersectionExitType == IntersectionExitType.LaneTransition && intersection.RoadConnections.Count != 2)
                        continue;
                    if (spawnObject.intersectionExitType == IntersectionExitType.RoadEnd && !intersection.IsEndObject())
                        continue;
                    ...
                    if (!intersection.IsEndObject())
                        if (!CheckDirections(road, intersection, spawnObject, tangent))
                            continue;

                    var spawns = new List<SpawnedObject>();

                    var spawnChance = CheckSpawnChance(intersection.roadDescr.settings, spawnObject.chance);
                    if (spawnChance) spawns.Add(Spawn(road.roadDescr, spawnObject, position, tangent, upVector, false, true));

                    if (spawnObject.position == SpawnObjectPosition.BothSides)
                    {
                        spawnChance = CheckSpawnChance(intersection.roadDescr.settings, spawnObject.chance);
                        if (spawnChance)
                    ...
                    if (spawnObject.removeOverlap)
                        for (var k = spawns.Count - 1; k >= 0; k--)
                            if (!CheckOverlap(spawns[k].gameObject, roadObjects, intersectionBounds,
                                    overlapIntersectionIndexes, overlapRoadIndexes))
                            {
                                ObjectUtility.DestroyObject(intersection.roadDescr.roadConstructor.pool, spawns[k]);
                                spawns.RemoveAt(k);
                            }
        ...
        private static List<SpawnedObject> SpawnRailing(List<SpawnObjectPreset> spawnObjectPresets, SceneObject sceneObject)
        {
            ...
            foreach (var spline in railingSplines)
            {
                ...
                foreach (var spawnObject in spawnObjects)
                {
                    ...
                    if (sceneObject is RoadObject road)
                        if (road.curvature < spawnObject.curvature.x || road.curvature > spawnObject.curvature.y)
                            return spawnedObjects;
                    if (sceneObject is IntersectionObject intersection)
                        if (intersection.IsEndObject() && !spawnObject.railingRoadEnds)
                            return spawnedObjects;
        ...
        /********************************************************************************************************************************/
        // Serialized Spawned Objects

        internal static List<SerializedSpawnedObject> CreateSerializedSpawns(SceneObject sceneObject)
        {
            var serializedSpawns = new List<SerializedSpawnedObject>();

            var roadSet = sceneObject.roadDescr.roadConstructor._RoadSet;
            var spawnedObjects = sceneObject.GetSpawnedObjects();

            for (var i = 0; i < spawnedObjects.Length; i++)
            {
                var spawnedObject = spawnedObjects[i];
                var spawnObject = spawnedObject.spawnObject;

                var found = false;

                for (var j = 0; j < roadSet.spawnObjectPresets.Count && !found; j++) // Preset spawn objects
                {
                    var preset = roadSet.spawnObjectPresets[j];
                    for (var k = 0; k < preset.spawnObjects.Count; k++)
                        if (spawnObject.obj == preset.spawnObjects[k].obj)
                        {
                            AddSpawn(true, j, -1, k);
                            found = true;
                            break;
                ...
                if (!found)
                    for (var j = 0; j < roadSet.roads.Count && !found; j++) // Road-specific spawn objects
                    {
                        var roadSetRoad = roadSet.roads[j];
                        if (roadSetRoad.roadName != sceneObject.road.roadName) continue;

                        for (var k = 0; k < roadSetRoad.spawnObjects.Count; k++)
                            if (spawnObject.obj == roadSetRoad.spawnObjects[k].obj)
                            {
                                AddSpawn(false, -1, j, k);
                                found = true;
                                break;
                            }
                    }

                void AddSpawn(bool isPreset, int presetIndex, int roadIndex, int objectIndex)
                {
                    var serializedSpawn = new SerializedSpawnedObject();
                    serializedSpawn.isPreset = isPreset;
                    serializedSpawn.presetIndex = presetIndex;
                    serializedSpawn.roadIndex = roadIndex;
                    serializedSpawn.objectIndex = objectIndex;
                    serializedSpawn.position = spawnedObject.transform.position;
                    serializedSpawn.rotation = spawnedObject.transform.rotation;
                    serializedSpawn.scale = spawnedObject.transform.localScale;
                    serializedSpawns.Add(serializedSpawn);
                }
            }
        ...
        internal static void SpawnSerializationLoad(SerializedSceneObject serializedBase, SceneObject sceneObject)
        {
            var roadDescr = sceneObject.roadDescr;
            var pool = roadDescr.roadConstructor.pool;
            var roadSet = roadDescr.roadConstructor._RoadSet;
            for (var i = 0; i < serializedBase.spawnedObjects.Count; i++)
            {
                var serializedSpawn = serializedBase.spawnedObjects[i];

                if (serializedSpawn.isPreset)
                {
                    if (serializedSpawn.presetIndex < roadSet.spawnObjectPresets.Count)
                    {
                        var preset = roadSet.spawnObjectPresets[serializedSpawn.presetIndex];
                        if (serializedSpawn.objectIndex < preset.spawnObjects.Count)
                        {
                            var presetSpawnObject = preset.spawnObjects[serializedSpawn.objectIndex];
                            var spawn = CreateSpawn(pool, presetSpawnObject, serializedSpawn.position, serializedSpawn.rotation,
                                serializedSpawn.scale);
                            spawn.transform.SetParent(sceneObject.transform);
                        }
                    }
                }
                else
                {
                    if (serializedSpawn.roadIndex < roadSet.roads.Count)
                    {
                        var roadSetRoad = roadSet.roads[serializedSpawn.roadIndex];
                        if (roadSetRoad.roadName == sceneObject.road.roadName)
                            if (serializedSpawn.objectIndex < roadSetRoad.spawnObjects.Count)
                            {
                                var presetSpawnObject = roadSetRoad.spawnObjects[serializedSpawn.objectIndex];
                                var spawn = CreateSpawn(pool, presetSpawnObject, serializedSpawn.position, serializedSpawn.rotation,
                                    serializedSpawn.scale);
                                spawn.transform.SetParent(sceneObject.transform);
                            }
            ...
            bool otherSide, bool roadEnd)
        {
            ...
            var spawnScale = Vector3.one;
            if (spawnObject.scale != Vector2.one)
            {
                var _scale = Random.Range(spawnObject.scale.x, spawnObject.scale.y);
                spawnScale = new Vector3(_scale, _scale, _scale);
            }

            var spawnedObject = CreateSpawn(roadDescr.roadConstructor.pool, spawnObject, spawnPosition, spawnRotation, spawnScale);

            spawnedObject.otherSide = otherSide;
            return spawnedObject;
        }

        private static SpawnedObject CreateSpawn(ConstructionPool pool, SpawnObject spawnObject, Vector3 position, Quaternion rotation, Vector3 scale)
        {
            var spawnedObj = pool.GetSpawn(spawnObject.obj, position, rotation);

            if (!spawnedObj.TryGetComponent<SpawnedObject>(out var spawnedObject))
            {
                spawnedObject = spawnedObj.AddComponent<SpawnedObject>();
        ...
        private static bool CheckSpawnChance(ComponentSettings settings, float chance)
        {
            if (settings.randomSeed)
            {
                var randomConst = Unity.Mathematics.Random.CreateFromIndex((uint) settings.seed);
                if (chance >= randomConst.NextFloat(0f, 1f)) return true;
            }
            else
            {
                if (chance >= Random.value) return true;
            }
