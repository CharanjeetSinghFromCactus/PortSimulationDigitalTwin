...
namespace PampelGames.RoadConstructor
{
    ...
    internal static class IntersectionCreation
    {
            ...
            RoadObject newRoadObject)
        {
            ...
            var snappedTrack = overlap.IsSnappedTrack();

            var knotDatas = new List<KnotData>();

            /********************************************************************************************************************************/
            /********************************************************************************************************************************/
            if (overlap.overlapType == OverlapType.Intersection || snappedTrack)
            {
                var connections = new List<RoadObject>();
                if (!snappedTrack) connections.AddRange(overlap.intersectionObject.RoadConnections);
                ...
                for (var i = 0; i < knotDatas.Count; i++)
                {
                    ...
                    var otherKnots = otherDatas.Select(t => t.nearestKnot).ToList();
                    var otherIndex = AngleDistanceUtility.GetIndexWithLeastDegrees(overlap.position, tangentIn, otherKnots);
                    ...
                    if (otherConnections[otherIndex].iD != newRoadObject.iD) // Don't reduce if tightest neighbour is existing neighbour.
                    {
                        ...
                        if (otherOtherDatas.Count > 0)
                        {
                            ...
                            var otherOtherKnots = otherOtherDatas.Select(t => t.nearestKnot).ToList();
                            var otherOtherIndex = AngleDistanceUtility.GetIndexWithLeastDegrees(overlap.position, tangentIn, otherOtherKnots);
            ...
            IntersectionValidation.ValidateKnotDatas(constructionFails, knotDatas);
        ...
        private static IntersectionObject CreateIntersectionInternal(List<KnotData> knotDatas, float3 centerPosition, bool elevated)
        {
            ...
            CreateIntersectionObject(priorityMeshData.roadDescr.roadConstructor.pool, priorityMeshData.roadDescr.road,
        ...
        public static void CreateIntersectionObject(ConstructionPool pool, Road road,
        ...
        {
            var intersectionObj = ObjectUtility.CreateObj(pool, Constants.PrefixIntersection, road.shadowCastingMode, out meshFilter, out meshRenderer);
        ...
        {
            var roadDescr = intersectionObject.roadDescr;
            var road = roadDescr.road;
            var newIntersectionObj = ObjectUtility.CreateObj(roadDescr.roadConstructor.pool, intersectionObject.NamePrefix(), road.shadowCastingMode, out var meshFilter,
            ...
            out List<Material> newMaterials, out List<Spline> newSplines, out List<float3> newPositions)
        {
            ...
            if(datas.Count == 0) return new Mesh();
            var pool = datas[0].roadDescr.roadConstructor.pool;
            ...
            CreateIntersectionRoadMesh(pool, roadLanesIntersection, leadingRoadDescr.settings, centerAverage, leadingRoadDescr.width,
            ...
            for (var i = 0; i < exitDatas.Count; i++)
            {
                ...
                CreateIntersectionRoadMesh(pool, exitLanes, leadingRoadDescr.settings, center,
            ...
            {
                var closingRectangleMesh = pool.GetMesh();
        ...
        private static void CreateIntersectionRoadMesh(ConstructionPool pool, List<Lane> lanes, ComponentSettings settings, float3 center,
            ...
            List<Mesh> newMeshes, List<Material> newMaterials, List<Spline> newSplines, float widthStart, float widthEnd)
        {
            ...
            RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanes, splineMeshParameter,
            ...
            out List<Mesh> newMeshes, out List<Material> newMaterials, float lodAmount)
        {
            ...
            if (sideLanesMain.Count != 0 && sideLanesSide.Count != 0)
            {
                ...
                {
                    var newMesh = roadDescrMain.roadConstructor.pool.GetMesh();
            ...
            if (sideLanesSide.Count == 0) // Closing rectangle
            {
                ...
                for (var i = 0; i < sideLanesMain.Count; i++)
                {
                    ...
                    var _newMesh = roadDescrMain.roadConstructor.pool.GetMesh();
        ...
        public static List<Spline> CreateRailingSplines(IntersectionObject intersection)
        {
            ...
            {
                if (settings.roadEnd == RoadEnd.None) return railingSplines;

                var intersectionSpline = intersection.splineContainer.Spline;
                var knot01 = intersectionSpline.Knots.First();
                ...
                var rotation = quaternion.LookRotation(tangent01perp, math.up());
                var radius = roadDescr.sideLanesCenterDistance + roadDescr.sideLanesWidth * 0.5f;
                var newSpline = SplineCircle.CreateHalfCircleSpline(radius, centerPosition, rotation, false);

                railingSplines.Add(newSpline);
