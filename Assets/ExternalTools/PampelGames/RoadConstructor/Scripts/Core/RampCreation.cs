...
namespace PampelGames.RoadConstructor
{
    ...
    {
        public static void CreateNewRamp(List<ConstructionFail> constructionFails, ConstructionObjects constructionObjects, Overlap overlap, RoadObject newRoadObject)
        {
            ...
            if (widthGapLeft + widthGapRight > oldRoadObject.length)
            {
            ...
            if (newRoadsRight.Count == 0 || newRoadsLeft.Count == 0)
            {
                constructionFails.Add(new ConstructionFail(FailCause.IntersectionTrackLength));
                return;
            }
            ...
            float3 centerPosition, bool elevated)
        {
            ...
            var rampObj = ObjectUtility.CreateObj(roadDescr_Road.roadConstructor.pool, Constants.PrefixRamp, roadDescr_Road.road.shadowCastingMode, out var meshFilter,
            ...
            out List<Material> newMaterials)
        {
            ...
            var pool = roadDescr_Road.roadConstructor.pool;
            ...
            var resolution =
                ConstructionSplineUtility.CalculateResolution(roadDescr_Road.resolution, settings.smartReduce, settings.smoothSlope, spline_Road[0],
                    spline_Road[^1], lodAmount);
            ...
            RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanesRoad, splineMeshParameterRoad,
            ...
            RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanesRampExit, splineMeshParameterRampExit,
            ...
            if (elevated)
            {
                var lanesRampExitElevated = roadDescr_Ramp.lanesElevatedOnly;

                var elevatedKnotLeft01 = new BezierKnot(positionRampLeft, -rampTangentIn, rampTangentIn, quaternion.identity);
                var elevatedKnotRight01 = new BezierKnot(positionRampRight, -rampTangentIn, rampTangentIn, quaternion.identity);

                if (!rampIn) (elevatedKnotLeft01, elevatedKnotRight01) = (elevatedKnotRight01, elevatedKnotLeft01);

                var splineRoadLength = spline_Road.GetLength();
                var widthT = roadDescr_Ramp.width / splineRoadLength;
                var elevatedPositionLeft02_T = 0.5f - widthT * 0.5f;
                var elevatedPositionRight02_T = 0.5f + widthT * 0.5f;
                var elevatedPositionLeft02 = spline_Road.EvaluatePosition(elevatedPositionLeft02_T);
                var elevatedPositionRight02 = spline_Road.EvaluatePosition(elevatedPositionRight02_T);

                var elevatedKnotLeft02 = new BezierKnot(elevatedPositionLeft02, -rampTangentIn, rampTangentIn, quaternion.identity);
                var elevatedKnotRight02 = new BezierKnot(elevatedPositionRight02, -rampTangentIn, rampTangentIn, quaternion.identity);

                if (!roadIsForward) (elevatedKnotLeft02, elevatedKnotRight02) = (elevatedKnotRight02, elevatedKnotLeft02);

                var elevatedSplineLeft = new Spline
                {
                    {elevatedKnotLeft01, TangentMode.Broken},
                    {elevatedKnotLeft02, TangentMode.Broken}
                };
                var elevatedSplineRight = new Spline
                {
                    {elevatedKnotRight01, TangentMode.Broken},
                    {elevatedKnotRight02, TangentMode.Broken}
                };

                var elevatedKnot01 = new BezierKnot(rampNearestKnot.Position, -rampTangentIn, rampTangentIn, quaternion.identity);
                var elevatedKnot02 = new BezierKnot(centerPosition, -rampTangentIn, rampTangentIn, quaternion.identity);
                var elevatedSpline = new Spline
                {
                    {elevatedKnot01, TangentMode.Broken},
                    {elevatedKnot02, TangentMode.Broken}
                };

                var splineMeshParameterRampExitElevated = new SplineMeshParameter(roadDescr_Ramp.width, roadDescr_Ramp.road.length,
                    resolution, settings.splineLengthUV, elevatedSpline, SplineLeftRight.Custom, elevatedSplineLeft, elevatedSplineRight);

                RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanesRampExitElevated, splineMeshParameterRampExitElevated,
            ...
            RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanesRampExit, splineMeshParameterRampClosingExit,
            ...
            out List<Mesh> rampSideMeshes, out List<Material> rampSideMaterials, out Spline sideSplineMiddle)
        {
            ...
            var detailResolution =
                ConstructionSplineUtility.CalculateResolution(roadDescr_Road.detailResolution, settings.smartReduce, settings.smoothSlope,
                    sideSplineMiddle[0], sideSplineMiddle[1],
                    lodAmount);
            ...
            RoadSplineMesh.CreateMultipleSplineMeshes(pool, lanesExitLeft, splineMeshParameterRampExit,
