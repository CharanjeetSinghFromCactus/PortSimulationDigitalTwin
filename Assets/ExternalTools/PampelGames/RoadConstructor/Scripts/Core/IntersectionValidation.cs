...
namespace PampelGames.RoadConstructor
{
    public static class IntersectionValidation
    {
        ...
        public static void ValidateKnotDatas(List<ConstructionFail> constructionFails, List<KnotData> knotDatas)
        {
            // Check if knot datas overlap with each other
            for (var i = 0; i < knotDatas.Count; i++)
            {
                var knotData = knotDatas[i];
                var width = knotData.roadDescr.width;

                for (var j = i + 1; j < knotDatas.Count; j++)
                {
                    var otherKnotData = knotDatas[j];
                    var otherWidth = otherKnotData.roadDescr.width;
                    var maxDistance = math.max(width, otherWidth) * 0.5f;
                    var distance = math.distance(knotData.nearestKnot.Position, otherKnotData.nearestKnot.Position);
