...
namespace PampelGames.RoadConstructor
{
    ...
    {
        public static List<ConstructionFail> ValidateRoad(ComponentSettings settings, ConstructionData roadData, RoadDescr roadDescr,
            Spline trackSpline, RoadConstructor.SceneData sceneData, List<Overlap> overlaps, bool canSpanOverConnectedTracks)
        {
            ...
            if (settings.validateOverlap && overlaps.Count > 1 && overlaps[0].exists && overlaps[1].exists)
            ...
            for (var i = 0; i < overlaps.Count; i++) ignoreObjects.AddRange(OverlapUtility.GetIgnoreObjects(overlaps[i]));
            ...
            {
                if (roadData.parallelRoad) break;
                var overlap = overlaps[i];
                if (!overlap.exists) continue;
                if (overlap.overlapType != OverlapType.Road) continue;
                if(overlap.roadObject.GetType() != typeof(RoadObject)) continue;

                GetRequiredDistances(overlap, roadDescr, trackSpline, 
                    out var requiredDistanceStart, out var requiredDistanceEnd);
                
                requiredDistanceStart += settings.roadLength.x;
                requiredDistanceEnd += settings.roadLength.x;

                var extendedStartLength = ConnectionUtility.GetConnectingLength(overlap.roadObject, overlap.t, requiredDistanceStart, false, canSpanOverConnectedTracks);
                var extendedEndLength = ConnectionUtility.GetConnectingLength(overlap.roadObject, overlap.t, requiredDistanceEnd, true, canSpanOverConnectedTracks);
                
                if (extendedStartLength <= requiredDistanceStart || extendedEndLength <= requiredDistanceEnd)
        ...
        private static void GetRequiredDistances(Overlap overlapRoad, RoadDescr newRoadDescr, Spline newRoadSpline,
            out float requiredDistanceStart, out float requiredDistanceEnd)
        {
            var oldRoad = overlapRoad.roadObject;
            var settings = oldRoad.roadDescr.settings;
            var newNearestKnotIndex = ConstructionSplineUtility.GetNearestKnotIndex(newRoadSpline, overlapRoad.position);
            var newNearestKnot = newRoadSpline.Knots.ElementAt(newNearestKnotIndex);
            var overlapTangent = overlapRoad.tangent;
            var newRoadTangent = newNearestKnot.TangentOut;
            if(newNearestKnotIndex != 0) newRoadTangent = -newRoadTangent;
            
            var angleLeft = PGTrigonometryUtility.AngleXZ(newRoadTangent, -overlapTangent);
            var angleRight = PGTrigonometryUtility.AngleXZ(newRoadTangent, overlapTangent);
            angleLeft = math.abs(angleLeft);
            angleRight = math.abs(angleRight);

            if (overlapRoad.IsSnappedTrack())
            {
                angleLeft = math.radians(90f);
                angleRight = math.radians(90f);
            }
            
            var widthExisting = newRoadDescr.width; // Need new here
            var widthConnecting = oldRoad.roadDescr.width; // Need old here
            
            requiredDistanceStart = AngleDistanceUtility.GetAngleDistance(settings.intersectionDistance, widthExisting, widthConnecting, angleLeft);
            requiredDistanceEnd = AngleDistanceUtility.GetAngleDistance(settings.intersectionDistance, widthExisting, widthConnecting, angleRight);
        }
        ...
        {
            if (!settings.validateOverlap)
            {
                splinePositions = Array.Empty<float3>();
                splineTangents = Array.Empty<float3>();
            ...
            for (var i = overlapIntersectionIndexes.Count - 1; i >= 0; i--)
            {
            ...
            float3[] splineTangents)
        {
            ...
            for (var i = 0; i < resultsArray.Length; i++)
            {
                ...
                if (!settings.validateHeight) continue;
