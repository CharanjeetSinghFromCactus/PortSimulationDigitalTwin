    ...
    public static class ConnectionUtility
    {
        ...
        private static void ClearConnections(List<SceneObject> sceneObjects)
        {
            for (var i = 0; i < sceneObjects.Count; i++)
            {
                ...
                {
                    var connectionPoints = customObject.connectionPoints;
                    for (var j = 0; j < connectionPoints.Count; j++) connectionPoints[j].used = false;
                }
        ...
        /// <summary>
        ///     Gets the total length of all connecting roads until the specified minLength is reached.
        /// </summary>
        /// <param name="isForwardDirection">If true, follows the road end. Otherwise follows the road start.</param>
        public static float GetConnectingLength(RoadObject road, float roadT, float minLength, bool isForwardDirection, bool canSpanOverConnectedTracks)
        {
            var spline = road.splineContainer.Spline;
            var spaceT = !isForwardDirection ? roadT : 1f - roadT;
            var totalLength = spaceT * road.length;

            if (totalLength >= minLength) return totalLength;
            if (spline.Count < 2) return totalLength;
            if(!canSpanOverConnectedTracks) return totalLength;

            var visited = new HashSet<RoadObject> {road};

            RoadObject initialRoadConnection = null;
            var roadConnections = road.RoadConnections;

            for (var i = 0; i < roadConnections.Count; i++)
            {
                var roadConnection = roadConnections[i];
                if (roadConnection.GetType() != typeof(RoadObject)) continue;

                var nearestKnot = ConstructionSplineUtility.GetNearestKnot(roadConnection.splineContainer.Spline, spline.GetBounds().center);

                if (isForwardDirection)
                {
                    var distanceEnd = math.distancesq(spline[^1].Position, nearestKnot.Position);
                    if (distanceEnd > Constants.MaxConnectionDistance) continue;
                    initialRoadConnection = roadConnection;
                    break;
                }

                var distanceStart = math.distancesq(spline[0].Position, nearestKnot.Position);
                if (distanceStart > Constants.MaxConnectionDistance) continue;
                initialRoadConnection = roadConnection;
                break;
            }


            if (!initialRoadConnection) return totalLength;

            void Traverse(RoadObject _road)
            {
                if (!_road) return;
                if (_road.GetType() != typeof(RoadObject)) return;
                if (!visited.Add(_road)) return; // avoid loops

                totalLength += _road.length;

                if (totalLength >= minLength) return;

                foreach (var next in _road.RoadConnections)
                {
            ...
            Traverse(initialRoadConnection);
        ...
        /// <summary>
        ///     Gets a list of all connecting roads until the specified minLength is reached.
        /// </summary>
        public static List<RoadObject> GetConnectingRoads(RoadObject startRoad, float minLength)
        {
            var result = new List<RoadObject>();
            var visited = new HashSet<RoadObject>();

            var totalLength = 0f;

            void Traverse(RoadObject road)
            {
                if (!road) return;
                if (road.GetType() != typeof(RoadObject)) return;
                if (!visited.Add(road)) return; // avoid loops
                ...
                foreach (var next in road.RoadConnections)
                {
                    if (!visited.Contains(next)) Traverse(next);
                    if (totalLength >= minLength) return;
                }
