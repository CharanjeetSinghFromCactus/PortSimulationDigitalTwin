...
namespace PampelGames.RoadConstructor
{
    internal static class RoadCreationData
    {
            ...
            out RoadObjectClass roadObjectClass, out Overlap overlap01, out Overlap overlap02)
        {
            ...
            var alignConnection = overlap01.exists && overlap02.exists && settings.alignConnections;
            ...
            void AlignOverlap(Overlap _overlap, ref float3 _position01, ref float3 _tangent01, ref float3 _position02, ref float3 _tangent02)
            {
                ...
                // Direct Connection, omit intersection or connection track.
                else if (_overlap.IsDirectConnection(roadDescr))
                {
                    _position01 = _overlap.position;
                    _tangent01 = _overlap.tangent;
                }
                else
                {
            ...
            if (overlap01.IsDirectConnection(roadDescr) && !PGTrigonometryUtility.IsSameDirectionXZ(tangent01, overlap01.tangent))
                tangent01 *= -1f;
            if (overlap02.IsDirectConnection(roadDescr) && !PGTrigonometryUtility.IsSameDirectionXZ(tangent02, overlap02.tangent))
                tangent02 *= -1f;

            if (roadSettings.setTangent01Length) tangent01 = math.normalize(tangent01) * roadSettings.tangent01Length;
            if (roadSettings.setTangent02Length) tangent02 = math.normalize(tangent02) * roadSettings.tangent02Length;
        ...
        private static float GetMinAngle(Overlap overlap, float3 tangent)
        {
            ...
            if (overlap.overlapType == OverlapType.Intersection || overlap.IsSnappedTrack())
            {
