...
namespace PampelGames.RoadConstructor
{
    ...
    public class RoadConstructor : ConstructionBase
    {
        ...
        internal ConstructionPool pool;
        ...
        private void InitializeScene()
        {
            ...
            pool = GetComponentInChildren<ConstructionPool>();
            if (!pool)
            {
                var poolObj = new GameObject(Constants.PoolParent).transform;
                pool = poolObj.gameObject.AddComponent<ConstructionPool>();
            }

            pool.Initialize(componentSettings.poolEnabled, componentSettings.poolSizeObjects, componentSettings.poolSizeSpawns,
                componentSettings.poolSizeMeshes);
        ...
        private void UninitializeInternal()
        {
            for (var i = gameObject.transform.childCount - 1; i >= 0; i--)
            {
                ...
                if (child.gameObject.TryGetComponent<ConstructionPool>(out var constructionPool)) continue;
                ObjectUtility.DestroyObjectDirect(child.gameObject);
            }

            ObjectUtility.DestroyObjectDirect(pool.gameObject);
        ...
        {
            sceneObject = null;
            var overlap = OverlapUtility.GetOverlap(componentSettings, searchRadius, float.MaxValue, position, sceneData,
                new List<ConstructionBase>());
        ...
        public override List<ConstructionFail> DetectTrackOverlap(Spline trackSpline, float trackWidth, float trackSpacing,
        ...
        public void ClearDisplayedObjects()
        {
            for (var i = 0; i < displayedObjects.Count; i++)
            {
                ...
                ObjectUtility.DestroyObject(pool, displayedObjects[i].gameObject);
            }
        ...
        {
            for (var i = 0; i < displayedDemolishObjects.Count; i++) ObjectUtility.DestroyObjectDirect(displayedDemolishObjects[i]);
        ...
        {
            for (var i = 0; i < displayedMoveIntersectionObjects.Count; i++) ObjectUtility.DestroyObjectDirect(displayedMoveIntersectionObjects[i]);
        ...
        public Vector3 SnapPosition(float radius, Vector3 position, out Overlap overlap)
        {
            ...
            overlap = OverlapUtility.GetOverlap(componentSettings, radius, float.MaxValue, newPosition, sceneData, integrations);
        ...
        {
            foreach (var undoObject in undoObjects) ObjectUtility.DestroyObject(pool, undoObject.gameObject);
            undoObjects.Clear();
        }
        ...
        {
            var _spline = new Spline(spline);
            ConstructionSplineUtility.NegateRotations(_spline);
            ...
            var roadSettings = new RoadSettings
            {
                ...
                setTangent01Length = true,
                tangent01Length = math.length(knot01.TangentOut),
                setTangent02Length = true,
                tangent02Length = math.length(knot02.TangentIn)
            };
            return DisplayRoadInternal(roadName, knot01.Position, knot02.Position, roadSettings);
        }
        ...
        /// <summary>
        ///     Demolishes the specified intersections and roads seamlessly, preserving the structure of adjacent objects.
        /// </summary>
        public void DemolishSeamless(List<IntersectionObject> intersections, List<RoadObject> roads)
        {
            DemolishObjectsInternal(intersections, roads, true);
        }
        ...
        public void UpdateColliders<T>(List<T> sceneObjects) where T : SceneObject
        {
            for (var i = 0; i < sceneObjects.Count; i++)
                if (sceneObjects[i].meshFilter.gameObject.TryGetComponent<MeshCollider>(out var meshCollider))
                    ObjectUtility.DestroyObject(pool, meshCollider);
        ...
        public void RemoveTrafficSystem()
        {
            ...
            for (var i = 0; i < sceneObjects.Count; i++)
            {
                ...
                {
                    ObjectUtility.DestroyObject(pool, sceneObject.traffic.gameObject);
        ...
        public void ConstructPreviewRoads(Transform parent, float roadLength, bool spawnObjects, bool elevated)
        {
            ...
            for (var i = 0; i < expandedRoads.Count; i++)
            {
                ...
                if (LOD) LODCreation.CreateLODs(pool, roadObject, componentSettings.lodList, road.shadowCastingMode);
        ...
        {
            var sceneObjects = GetSceneObjects();
            return GetSerializableRoadSystem(sceneObjects);
        }

        /// <summary>
        ///     Retrieves a list of serialized representations of the scene objects
        ///     within the railway system. Each object is converted into simple, serializable
        ///     fields for saving or transmission purposes.
        /// </summary>
        /// <param name="sceneObjects">A list of scene objects. Must be registered within the system.</param>
        /// <returns>A list of <see cref="SerializedSceneObject" /> representing the serialized railway system (incl. inheritors).</returns>
        public List<SerializedSceneObject> GetSerializableRoadSystem(List<SceneObject> sceneObjects)
        {
        ...
        public void AddSerializableRoadSystem(List<SerializedSceneObject> serializedSceneObjects)
        {
            ...
            var customSpawnObjects = true; // Recreate System button without serialized spawn objects
            ...
            for (var i = 0; i < serializedSceneObjects.Count; i++)
            {
                ...
                var _constructionObjects = new ConstructionObjects();
                serializedBase.CreateObjectFromSerializedData(_constructionObjects, roadDescr);

                if (serializedBase.spawnedObjects != null) customSpawnObjects = false;

                if (!customSpawnObjects)
                    for (var j = 0; j < _constructionObjects.CombinedNewObjects.Count; j++)
                    {
                        var sceneObject = _constructionObjects.CombinedNewObjects[j];
                        SpawnObjectUtility.SpawnSerializationLoad(serializedBase, sceneObject);
                    }

                constructionObjects.Merge(_constructionObjects);
            }

            FinalizeConstruction(constructionObjects, customSpawnObjects, false, true);

            /********************************************************************************************************************************/
            // Remove End Objects from existing connections.
            for (var i = 0; i < constructionObjects.newRoads.Count; i++) // Roads only have two connections
            {
                var _road = constructionObjects.newRoads[i];
                if (_road.Connections.Count == 2) continue;
                var _roadConnections = _road.RoadConnections;
            ...
            {
                var _intersection = constructionObjects.newIntersections[i];
                var _intersectionConnections = _intersection.IntersectionConnections;
            ...
            if (!customSpawnObjects)
            {
                // End Objects have no spawns (they are created in Finalize)
                var constructionEndObjects = new ConstructionObjects();
                for (var i = 0; i < constructionObjects.CombinedNewIntersections.Count; i++)
                    if (constructionObjects.CombinedNewIntersections[i].IsEndObject())
                        constructionEndObjects.newIntersections.Add(constructionObjects.CombinedNewIntersections[i]);

                SpawnObjects(constructionEndObjects);
            }
        }
        ...
        private ConstructionResultRoad DisplayRoadInternal(string roadName, float3 position01, float3 position02, RoadSettings roadSettings)
        {
            ...
            for (var i = 0; i < result.newReplacedRoads.Count; i++) ObjectUtility.DestroyObject(pool, result.newReplacedRoads[i].gameObject);
            ...
            bool finalize)
        {
            ...
            {
                constructionObjects.DestroyNewObjects(pool);
                return result;
            }
            ...
            RoadObject roadObject, ConstructionData roadData, Overlap _overlap, bool finalize)
        {
            ...
            if (_overlap.IsDirectConnection(roadDescr))
            {
                constructionObjects.removableIntersections.Add(_overlap.intersectionObject);
                return;
            }
            ...
            if (_overlap.overlapType == OverlapType.Shared)
            {
                ...
                CustomObjectCreation.CreateCustomObject(constructionObjects, roadDescr, sharedObject!.GetConstructionSplines(),
                    sharedObject.GetConnectionPoints(), sharedObject.IsElevated());
            }
            else
            {
            ...
            /********************************************************************************************************************************/
            // Validate if start and end have both modified the same existing objects
            constructionFails.AddRange(constructionObjects.ValidateDuplicates());
        }
            ...
            bool finalize)
        {
            ...
            {
                if (overlap01.overlapType == OverlapType.Road)
                    RampCreation.CreateNewRamp(constructionFails, constructionObjects, overlap01, roadObject);
                if (overlap02.overlapType == OverlapType.Road)
                    RampCreation.CreateNewRamp(constructionFails, constructionObjects, overlap02, roadObject);
            }
            ...
            bool spawnObjects, bool registerUndo, bool updateTerrain = false)
        {
            ...
            if (registerUndo)
                UndoObjectUtility.RegisterUndo(componentSettings, undoParent, undoObjects, constructionObjects, pool);
            else constructionObjects.DestroyRemovableObjects(pool);
        }
        ...
        private void ApplyComponentSettings<T>(List<T> newSceneObjects) where T : SceneObject
        {
            ...
            if (LOD)
                for (var i = 0; i < newSceneObjects.Count; i++)
                    LODCreation.CreateLODs(newSceneObjects[i].roadDescr.roadConstructor.pool, newSceneObjects[i], componentSettings.lodList,
                        newSceneObjects[i].road.shadowCastingMode);
        ...
        {
            var constructionObjectsGrounded = constructionObjects.IsolateElevated(constructionObjects, false);
            var constructionObjectsElevated = constructionObjects.IsolateElevated(constructionObjects, true);

            SpawnObjectsInternal(constructionObjectsGrounded);
            SpawnObjectsInternal(constructionObjectsElevated);
        }

        private void SpawnObjectsInternal(ConstructionObjects constructionObjects)
        {
        ...
        private void AddDisplayObjects<T>(List<T> sceneObjects) where T : SceneObject
        {
            ...
            for (var i = 0; i < sceneObjects.Count; i++)
                if (!displayedObjects.Contains(sceneObjects[i]))
                    displayedObjects.Add(sceneObjects[i]);
        }
        ...
        private List<GameObject> DisplayDemolishObjectsInternal(Vector3 position, float searchRadius, bool deactivateSceneObjects)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < overlap.roadObject.RoadConnections.Count; i++)
                {
                    ...
                    if (!roadConnection.IsSnappedTrack()) continue;
        ...
        private void DemolishObjectsInternal(List<IntersectionObject> demolishIntersections, List<RoadObject> demolishRoads, bool seamless = false)
        {
            var constructionObjects = new ConstructionObjects();

            /********************************************************************************************************************************/

            if (!seamless) PampelGames.RoadConstructor.Demolish.UpdateSceneObjects(constructionObjects, demolishIntersections, demolishRoads);

            /********************************************************************************************************************************/
            if (seamless && componentSettings.addTrafficComponent)
            {
                for (var i = 0; i < demolishIntersections.Count; i++)
                {
                    for (var j = 0; j < demolishIntersections[i].IntersectionConnections.Count; j++)
                    {
                        var _connection = demolishIntersections[i].IntersectionConnections[j];
                        if (_connection.IntersectionConnections.Contains(demolishIntersections[i]))
                            _connection.IntersectionConnections.Remove(demolishIntersections[i]);
                    }

                    for (var j = 0; j < demolishIntersections[i].RoadConnections.Count; j++)
                    {
                        var _connection = demolishIntersections[i].RoadConnections[j];
                        if (_connection.IntersectionConnections.Contains(demolishIntersections[i]))
                            _connection.IntersectionConnections.Remove(demolishIntersections[i]);
                    }
                }
                ...
                for (var i = 0; i < demolishRoads.Count; i++)
                {
                    for (var j = 0; j < demolishRoads[i].RoadConnections.Count; j++)
                    {
                        var _connection = demolishRoads[i].RoadConnections[j];
                        if (_connection.RoadConnections.Contains(demolishRoads[i])) _connection.RoadConnections.Remove(demolishRoads[i]);
                    }

                    for (var j = 0; j < demolishRoads[i].IntersectionConnections.Count; j++)
                    {
                        var _connection = demolishRoads[i].IntersectionConnections[j];
                        if (_connection.RoadConnections.Contains(demolishRoads[i])) _connection.RoadConnections.Remove(demolishRoads[i]);
                    }
                }
            }
            ...
            /********************************************************************************************************************************/
            if (!seamless && componentSettings.terrainSettings && componentSettings.demolishResetsTerrain)
            {
