...
namespace PampelGames.RoadConstructor
{
    ...
    public class RoadObject : SceneObject
    {
        ...
        {
            var serializedSpawns = SpawnObjectUtility.CreateSerializedSpawns(this);
            
            return new SerializedRoad(road.roadName, elevated, serializedSpawns, rampRoad, splineContainer.Spline, splitOriginalID, splitOriginalSpline);
        }
        
        /********************************************************************************************************************************/

        public bool IsSnappedTrack()
        {
            var settings = roadDescr.settings;
            if (curvature < settings.snapTrackMinAngle) return false;
            var maxSnapLength = Width * settings.snapTrackMaxLength; // Approximation
            return maxSnapLength > length;
        }

        public Vector3 SnappedTrackPosition()
        {
            var _spline = splineContainer.Spline;
            var knot01 = _spline[0];
            var knot02 = _spline[^1];
