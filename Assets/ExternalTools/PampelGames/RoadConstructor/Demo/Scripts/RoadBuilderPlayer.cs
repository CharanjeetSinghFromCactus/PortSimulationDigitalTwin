...
namespace PampelGames.RoadConstructor.Demo
{
    ...
    public class RoadBuilderPlayer : RoadBuilderBase
    {
        ...
        private void Update()
        {
            ...
            var ray = Camera.main.ScreenPointToRay(PGHybridInput.MousePosition);
            ...
            if (PGHybridInput.GetKey(increaseHeight)) SetDeltaHeight(deltaHeight + Time.deltaTime * deltaSpeed);
            if (PGHybridInput.GetKey(decreaseHeight)) SetDeltaHeight(deltaHeight - Time.deltaTime * deltaSpeed);
            if (PGHybridInput.GetKey(increaseRadius)) SetRadius(roundAboutRadius + Time.deltaTime * deltaSpeed);
            if (PGHybridInput.GetKey(decreaseRadius)) SetRadius(roundAboutRadius - Time.deltaTime * deltaSpeed);
            ...
            if (Input.GetKeyDown(KeyCode.Mouse1) || Input.GetKeyDown(detachRoad)) ResetValues();
        ...
        private void ButtonMenuClicked(string buttonMenuName)
        {
            ...
            for (var i = 0; i < _constructionSet.roads.Count; i++)
            {
                ...
                roadButton.text = string.Concat(roadButton.text.Select((c, _i) =>
                    char.IsUpper(c) && _i != 0 && roadButton.text[_i - 1] != '-'
                        ? "\n" + c
                        : c.ToString()
                ));
